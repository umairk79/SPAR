        ...
        $teacher = $_SESSION['id'];
             ...
             $link = mysqli_connect("localhost", "root", "","spar");
                ...
                else if(isset($_POST['submit5']))
                {
                    ...
                    $q="select * from comments where teacherid='".$teacher."' and studentid='".$thisid."'";
					$qres=mysqli_query($link,$q);
					
					if(mysqli_num_rows($qres)>0){
                    $query = "update comments set comment ='".$comment."',subject ='".$subject."' where teacherid='".$teacher."'";
                    $res = mysqli_query($link, $query);
                
                }
                    else{
                        $query = "insert into comments (studentid,teacherid,subject,comment) values('$thisid','$teacher','$subject','$comment')";
                        $res = mysqli_query($link, $query);
                    }
